...
@section('title_nm','Adaptor::Update Profile')
...
<h1 class="text-2xl font-bold text-[#515279] text-center mt-5">Edit Adaptor Profile</h1>
...
@if(session('success'))
        ...
        @endif
<form action="{{route('adaptor-editprofile-image')}}" class="flex flex-col items-start justify-center px-8 mt-8 space-y-7 md:px-72" enctype="multipart/form-data" method="POST">
    <a href="{{route('adaptor-my-profile')}}" class="font-bold text-blue-500">Back</a>
    @csrf
    <div class="image space-y-4 flex flex-col">
        ...
        <img src="/storage/{{Auth::guard('adaptor')->user()->image}}" alt="" class="h-10 w-10">
...
<form action="{{route('adaptor-editprofile-one')}}" class="flex flex-col items-start justify-center px-8 mt-8 space-y-7 md:px-72" enctype="multipart/form-data" method="POST">
  ...
  <div class="image space-y-4 flex flex-col">
    ...
    border-red-600 @enderror" value="{{Auth::guard('adaptor')->user()->full_name}}">
    @error('name')
    <p class="text-red-500">{{$message}}</p>
  @enderror
  </div>
  <div class="image space-y-4 flex flex-col">
    ...
    <input type="email" disabled  id="email" name="email" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline md:w-[34rem] @error('email')
    border-red-600 @enderror" value="{{Auth::guard('adaptor')->user()->email}}">
    @error('email')
    <p class="text-red-500">{{$message}}</p>
  @enderror
  </div>
  <div class="image space-y-4 flex flex-col">
    ...
    border-red-600 @enderror" value="{{Auth::guard('adaptor')->user()->mobile_no}}">
    @error('mobile')
    <p class="text-red-500">{{$message}}</p>
  @enderror
  </div>
  <div class="image space-y-4 flex flex-col">
    ...
    border-red-600 @enderror" value="{{Auth::guard('adaptor')->user()->address}}">
    @error('address')
    <p class="text-red-500">{{$message}}</p>
  @enderror
  </div>
  <div class="image space-y-4 flex flex-col">
    ...
    border-red-600 @enderror">
        <option selected value="{{Auth::guard('adaptor')->user()->state}}">{{Auth::guard('adaptor')->user()->state}}</option>
  ...
  <div class="image space-y-4 flex flex-col">
    ...
    border-red-600 @enderror" value="{{Auth::guard('adaptor')->user()->pin_code}}">
    @error('pin_code')
    <p class="text-red-500">{{$message}}</p>
  @enderror
  </div>
